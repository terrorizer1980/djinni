...
package object generatorTools {
  ...
  case class Spec(
                   ...
                   jsExt: String,
                   jsOutFolder: Option[File],
                   jsBaseLibIncludePrefix: String,
  ...
  def generate(idl: Seq[TypeDecl], spec: Spec): Option[String] = {
    try {
      ...
      if (spec.jsOutFolder.isDefined) {
        if (!spec.skipGeneration) {
          createFolder("Js", spec.jsOutFolder.get)
        }
        new JsGenerator(spec).generate(idl)
      }
...
abstract class Generator(spec: Spec)
{
  ...
  def writeJsFileGeneric(folder: File, namespace: String, fileIdentStyle: IdentConverter)(name: String, origin: String, includes: Iterable[String], f: IndentWriter => Unit) {
    createFile(folder, fileIdentStyle(name) + "." + spec.jsExt, (w: IndentWriter) => {
      ...
      w.wl(s"#include ${q("emscripten/bind.h")}")
      w.wl
